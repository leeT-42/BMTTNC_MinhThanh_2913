        ...
        while True:
            ...
            for client in clients:
                if client != client_socket:
                    try:
                        client.send(data)
                    except: # Handles cases where a client might have disconnected abruptly
...
while True:
    ...
    # Tạo SSL context
    context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER) # Changed to PROTOCOL_TLS_SERVER for newer Python versions

    # Load certificate and key
    # Make sure 'certificates' folder is in the same directory as this script
    # and contains 'server-cert.crt' and 'server-key.key'
